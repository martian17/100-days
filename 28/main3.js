...
var calc = function(){
    ...
    .map(function(a){
        ...
        }else if(a.slice(0,5) == "solve"){
            solveVars = a.slice(5).trim().split(" ").map(a=>a.trim());
        }
    });
    console.log(JSON.stringify(variables));
    console.log(JSON.stringify(cummed));
    console.log(JSON.stringify(solveVars));

    //cummed = cummed.map(c=>stringify(cleanAST(expand(reducePower(assign(variables,c),false)))));
    cummed = cummed.map(c=>splitTerms(expand(reducePower(assign(variables,c),false)),solveVars))
...
var splitTerms = function(eq,vars){
    ...
    termized.push(eq);
    console.log(termized);
    return termized;
};

var selectTerm = function(eq,vari){
    if(eq[0] !== "+"){
...
var compact = function(eq){//one dimensional compaction
    ...
    console.log(JSON.stringify(varss));

    var common_vars = varss.reduce(function(a,b){
        var c = [];
        var j = 0;
    ...
    varss = varss.map(function(a){
        var j = 0;
        var c = [];
        for(var i = 0; i < a.length; i++){
            if(a[i] === common_vars[j]){
                j++;
            }else{
                c.push(a[i]);
            }
    ...
    console.log(JSON.stringify(common_vars));

    var ret = ["*"];
    for(var i = 0; i < common_vars.length; i++){
        ret.push(["id",common_vars[i]]);
    }
    ret.push(eq);
...
var before = "(a^2-b+(c+d)*(sin((i+j)/(k+l))+f))*(g-h)";
console.log(before);

var parsed = parse(before);
console.log(parsed);

var reduced = reducePower(parsed);
console.log(reduced);

var expanded = expand(reduced);
