...
var blur = function(arr,dims,n){//destructive on arr
    //classical blur
...
var noiseGenerator = function(dims,r){
    var mul = dims.reduce((a,b)=>a*b);
    var arr = [];
    for(var i = 0; i < mul; i++){
        arr[i] = Math.random();
    }
    return blur(arr,dims,20);/*
    ...
    var mul = dims.map(a=>Math.ceil(a/r*planck)).reduce((a,b)=>a*b);
    var arr = [];
    for(var i = 0; i < mul.length; mul++){
        arr[i] = Math.random();
    }
    var small = blur(arr,dims.map(a=>Math.ceil(a/r*planck)),pp);
    ...
    for(var i = 0; i < mul0; i++){
        for(var j = 0; j < ndim; j++){
            //calculate the dim avg
            var cn = stack[j]/r*planck;
            var cnb = Math.floor(cn);
            var cnt = Math.ceil(cn);
            var avg =
