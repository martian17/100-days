...
var len = 300;
//init grid
var gridLen = Math.sqrt(width*height/len)*2;
var grid = [];
for(var i = 0; i < height/gridLen+1; i++){
    grid[i] = [];
    for(var j = 0; j < width/gridLen+1; j++){
        grid[i][j] = [];
    }
}
...
for(var i = 0; i < len; i++){
    ...
    var col = gradient([224,99,39],[64,16,125],y/height);
    var point = [x,y,col];
    points.push(point);
...
var closestPoint = function(x,y){
    ...
    while(true){
        ...
        var mindis = Infinity;
        for(var i = gridY-searchArea; i < gridY+searchArea+1; i++){
            if(!(i in grid))continue;
            for(var j = gridX-searchArea; j < gridX+searchArea+1; j++){
                if(!(j in grid[i]))continue;
                //console.log(i,j);
                ...
                for(var k = 0; k < box.length; k++){
                    ...
                    if(distp2 < mindistArea2 && distp2 < mindis){
                        mindis = distp2;
                        minPoint = point;
                    }
                }
