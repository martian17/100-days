...
var calc = function(){
    ...
    cummed = cummed.map(c=>expand(assign(variables,c),false));
...
var expand = function(eq,negativity){
    ...
    if(eq[0] === "-"){
        ...
        for(var i = 1; i < eq.length; i++){
            ...
            negativity = false;//only the first term
            if(first){
                arr = ans;
                first = false;
            }else{
                ...
                for(var j = 0; j < arr.length; j++){
                    for(var k = 0; k < ans.length; k++){
                        arr1.push(multiply(arr[j],ans[k]));
                    }
                }
                arr = arr1;
            }
        ...
        console.log([[negativity,eq]]);
        return [[negativity,eq]];
    }
};

var multiply = function(eq1,eq2){
    var sign = eq1[0]?!eq2[0]:eq2[0];//negativity
    var eq = [sign];
