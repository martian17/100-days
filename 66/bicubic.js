...
var splined = function(f0,f1,f0d,f1d,x){
    var a = 2*f0-2*f1+f0d+f1d;
    var b = -3*f0+3*f1-2*f0d-f1d;
    var c = f0d;
    var d = f0;
...
var cubic1d = function(itn,slo,w,ww){
    var result = [];
    for(var i = 0; i < ww; i++){
        var idx = Math.floor(i/ww*(w-1));
        var left = idx;
        var right = idx+1;
        var didx = i/ww*(w-1) - idx;
        result.push(spline(itn[left],itn[right],slo[left],slo[right],didx));
    }
    return result;
};

var cubic1dd = function(itn,slo,w,ww){
    var result = [];
    for(var i = 0; i < ww; i++){
        var idx = Math.floor(i/ww*(w-1));
        var left = idx;
        var right = idx+1;
        var didx = i/ww*(w-1) - idx;
        result.push(splined(itn[left],itn[right],slo[left],slo[right],didx));
    }
    return result;
}
