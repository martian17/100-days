...
var step = function(){
    ...
    for(var y = 0; y < height; y++){
        for(var x = 0; x < width; x++){
            ...
            var sum = 0;
            for(var i = 0; i < 4; i++){
                pressure[i] = pressure0[i]-pressure1[i];
                if(pressure[i] < 0){
                    pressure[i] = 0;
                }
                sum += pressure[i];
            ...
            put(shadow[Idx(x,y+1)],pressure[0],0,1);
            put(shadow[Idx(x-1,y)],pressure[0],-1,0);
            put(shadow[Idx(x+1,y)],pressure[0],1,0);
        }
    }
    var temp = shadow;
...
var render = function(){
    ...
    for(var i = 0; i < arr.length; i++){
        data[i*4+0] = Math.floor(arr[i][0]*255);
        data[i*4+1] = Math.floor(arr[i][1]*255);
        data[i*4+2] = Math.floor(arr[i][2]*255);
        data[i*4+3] = 255;
    }
