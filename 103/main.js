...
var pipokunI = document.createElement("img");
pipokunI.setAttribute("src","img.png");
pipokunI.style.display = "none";
document.body.appendChild(pipokunI);
var acc = 1;
var pc = [Math.random()*width,Math.random()*height,Math.random()-0.5,Math.random()-0.5,randColor()];
var pipokun = function(dt){
    acc *= 1.01;
    var x = pc[0]+pc[2]*dt/5*acc;
    var y = pc[1]+pc[3]*dt/5*acc;
    var vx = pc[2];
    var vy = pc[3];
    if(x < 0){
        vx = Math.abs(vx);
    ...
    pc = [x,y,vx,vy];
    ctx.drawImage(pipokunI, pc[0], pc[1]);
};
